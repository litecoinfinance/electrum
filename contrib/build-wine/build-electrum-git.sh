...
pushd $WINEPREFIX/drive_c/electrum-ltfn
...
rm -rf "$WINEPREFIX/drive_c/electrum-ltfn/electrum/locale/"
for i in ./locale/*; do
    dir="$WINEPREFIX/drive_c/electrum-ltfn/electrum/$i/LC_MESSAGES"
    mkdir -p $dir
...
info "Pip installing Electrum-LTFN. This might take a long time if the project folder is large."
$WINE_PYTHON -m pip install --no-dependencies --no-warn-script-location .
popd
...
cd dist
mv electrum-ltfn-setup.exe $NAME_ROOT-$VERSION-setup.exe
cd ..
