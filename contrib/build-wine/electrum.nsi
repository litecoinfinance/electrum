...
;Variables
  ...
  !define PRODUCT_NAME "Electrum LitecoinFinance"
  !define PRODUCT_WEB_SITE "https://github.com/litecoinfinance/electrum-ltfn"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-ltfn-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-ltfn\electrum\gui\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-ltfn\*.*"
  File "c:\electrum-ltfn\electrum\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-ltfn-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-ltfn-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-ltfn-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-ltfn-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-ltfn-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\litecoinfinance" "" "URL:litecoinfinance Protocol"
  WriteRegStr HKCU "Software\Classes\litecoinfinance" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\litecoinfinance" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\litecoinfinance\shell\open\command" "" "$\"$INSTDIR\electrum-ltfn-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\litecoinfinance"
