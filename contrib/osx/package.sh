...
if [ -z "$1" ]; then
    echo "Usage: $0 Electrum-LTFN.app"
    exit -127
fi
...
dmg dmg Electrum_uncompressed.dmg electrum-ltfn-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum_uncompressed.dmg

echo "Done."
sha256sum electrum-ltfn-$VERSION.dmg
